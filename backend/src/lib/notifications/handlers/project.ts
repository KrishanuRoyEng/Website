...
export async function notifyNewProjectSubmission(project: {
  ...
  try {
    ...
    await sendDiscordNotification(NOTIFICATION_CONFIG.discord.newProjects, {
...
export async function notifyProjectRejected(
  ...
  try {
    ...
    // Send Discord notification to announcement channel
    await sendDiscordNotification(
      NOTIFICATION_CONFIG.discord.projectRejections,
      {
        title: "✅ Rejected",
        description: `"${project.title}" has been rejected. Reason: ${reason}`,
